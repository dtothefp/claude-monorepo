...
If the import succeeds, print nothing and continue.

**Step 1b - Detect project root and cd into it**

Run this BEFORE creating any `graphify-out/` directories. This ensures `graphify-out/` lands in the project root rather than wherever the user's shell happens to be.
...
# Walk up from the input path looking for a project root marker
search = input_path if input_path.is_dir() else input_path.parent
project_root = None
p = search
while p != p.parent:
...
if project_root and project_root != Path.cwd():
    ...
    print(str(Path.cwd()))
" > /tmp/.graphify_root.txt

GRAPHIFY_ROOT=$(cat /tmp/.graphify_root.txt)
echo "Output root: $GRAPHIFY_ROOT"
cd "$GRAPHIFY_ROOT"
...
After cding into the project root, all `graphify-out/` paths are relative to that root. If no project root marker is found, the current directory is used (backward-compatible fallback).
...
**Critical: each subagent MUST write its JSON to disk, not return it inline.** If a subagent returns its JSON in the reply text, that text lives only in the parent conversation context — and if the parent compacts before Step B3 runs, the result is lost and the subagent has to be re-dispatched, doubling token cost. Writing to disk first makes the pipeline survive compaction. The reply text is just a status string for human readability.

```
You are a graphify extraction subagent. Read the files listed and extract a knowledge graph fragment.

**Output protocol — read carefully:**
1. Build the JSON object matching the schema below.
2. Write the JSON using the Write tool to: graphify-out/.subagent_chunkCHUNK_NUM.json
3. Reply with EXACTLY one line in this format: `done. N nodes, M edges, K hyperedges` (substitute the actual counts).
4. Do NOT include the JSON in your reply text. Do NOT add explanation, markdown fences, or preamble. The file on disk is the canonical output; the reply is only a status acknowledgement.

If you cannot write the file for any reason, reply with: `error: <reason>` and do not retry. The parent will handle the failure.
...
JSON schema to write to graphify-out/.subagent_chunkCHUNK_NUM.json:
...
Remember: write this JSON to disk via the Write tool, then reply with only the one-line status. Do not paste the JSON into your reply.
```

**Step B3 - Collect, cache, and merge**

Wait for all subagents. Each subagent should have written its JSON to `graphify-out/.subagent_chunkN.json` and replied with a one-line `done. ...` status. Do NOT parse JSON out of the subagent reply text — read it from the file on disk. This is what makes the pipeline survive a parent compaction mid-run.

For each chunk file:
- If `graphify-out/.subagent_chunkN.json` exists and parses, include it and save its files' nodes/edges to the cache.
- If the file is missing or invalid, print a warning and skip that chunk — do not abort. Check the subagent's reply text for an `error: ...` line so the warning is informative.

If more than half the chunks failed, stop and tell the user.

Combine all `graphify-out/.subagent_chunk*.json` files into `graphify-out/.graphify_semantic_new.json`, then save to cache:
```bash
$(cat graphify-out/.graphify_python) -c "
import json, glob
...
chunks = sorted(glob.glob('graphify-out/.subagent_chunk*.json'))
...
new = {'nodes': all_nodes, 'edges': all_edges, 'hyperedges': all_hyperedges, 'input_tokens': 0, 'output_tokens': 0}
...
print(f'Combined {len(chunks)} chunks: {len(all_nodes)} nodes, {len(all_edges)} edges; cached {saved} files')

# Clean up per-chunk scratch files now that they are merged + cached
for cf in chunks:
    Path(cf).unlink()
"
```
...
"
rm -f graphify-out/.graphify_detect.json graphify-out/.graphify_extract.json graphify-out/.graphify_ast.json graphify-out/.graphify_semantic.json graphify-out/.graphify_analysis.json graphify-out/.graphify_labels.json graphify-out/.graphify_python
